...
<div class="container">
    <form action="registration.php" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
        ...
        <input type="text" id="wnumber" name="wnumber" required pattern="^\+254\d{9}$" placeholder="Eg. +254792647563">

        <label for="password">Password(6 characters; symbols, letters and numbers):</label>
        <input type="password" id="password" name="password" required pattern="(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&*]).{6,}" placeholder="Eg. john@12">
...
<script>
    ...
    function validatePersonalFields() 
    {
        ...
        if (!/^(\+254)\d{9}$/.test(wnumber)) 
            {
                alert('WhatsApp number must start with "+254" and have 9 additional digits.');
                return false;
            }
        ...
        if (!/(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&*]).{6,}/.test(password)) 
            {
                alert('Password must be at least 6 characters long and include symbols, letters, and numbers.');
                return false;
            }
